window.addEventListener("load", function(){
	...
	// slide
	let visual_Slide_01 = visual_Slide(".visual", 5000, "fade");
	let visual_Slide_02 = visual_Slide(".product", 0, "fade");
	...
	// .projects
	function project_act(_targetWrap){
		const $wrap = _targetWrap;
		const $img_wraps = document.querySelectorAll(_targetWrap +" .img_wrap");
		const $desc_wrap = document.querySelector(_targetWrap +" .desc_wrap");
		const $btn_prev = document.querySelector(_targetWrap +" button.prev_arrow");
		const $btn_next = document.querySelector(_targetWrap +" button.next_arrow");
		const $elm_cur = document.querySelector(_targetWrap +" span#cur");
		let $pn_btns;
		if(isMobile && screen.width < 768) $pn_btns = document.querySelectorAll(_targetWrap +" .img_pn_wrap > button");
		let cnt = 0;

		$btn_prev.onclick = function(){
			cnt = cnt == 0 ? $desc_wrap.children.length - 1 : cnt - 1;
			change(cnt);
		}
		$btn_next.onclick = function(){
			cnt = cnt == $desc_wrap.children.length - 1 ? 0 : cnt + 1;
			change(cnt);
		}
		...
		if($pn_btns && $pn_btns.length){
			for(var i = 0; i < $pn_btns.length; i++){
				...
				$pn_btns[i].onclick = function(){
					//if(cnt == this.index) return;
					pn_change(this.index);
					img_change(this.index);
				}
			}
			function img_change(_num){
				for(var i = 0; i < $desc_wrap.children.length; i++){
					$img_wraps[i].classList.remove("active");
					for(var j = 0; j < $desc_wrap.children.length; j++){
						$img_wraps[i].children[j].classList.remove("active");
					}
				}
				$img_wraps[_num].classList.add("active");
				$img_wraps[_num].children[cnt].classList.add("active");
			}
			function pn_change(_num){
				for(var i = 0; i < $pn_btns.length; i++){
					$pn_btns[i].classList.remove("active");
		...
		function change(_num){
			if(isMobile && screen.width < 768){
				for(var i = 0; i < $img_wraps.length; i++){
					$img_wraps[i].classList.remove("active");
				}
				$img_wraps[_num].classList.add("active");
				img_change(0);
				...
				for(var i = 0; i < $desc_wrap.children.length; i++){
					$desc_wrap.children[i].classList.remove("active");
					for(var j = 0; j < $desc_wrap.children.length; j++){
						$img_wraps[i].children[j].classList.remove("active");
					}
				}
				for(var i = 0; i < $desc_wrap.children.length; i++){
					$img_wraps[i].children[_num].classList.add("active");
				}
				$desc_wrap.children[_num].classList.add("active");
			}
			$elm_cur.textContent = "0"+ (_num + 1);
		}
	}

	project_act(".projects");


	if(!isMobile){

		// swipe
		const ulWrap = document.querySelector('section.category > div.ul_wrap > ul');
		...
		ulWrap.addEventListener('mousedown', (e) => {
			isDragging = true;
			startX = e.pageX - ulWrap.offsetLeft;
			scrollLeft = ulWrap.scrollLeft;
		});

		ulWrap.addEventListener('mouseup', () => {
			isDragging = false;
		});

		ulWrap.addEventListener('mouseleave', () => {
			isDragging = false;
		});

		ulWrap.addEventListener('mousemove', (e) => {
			if (!isDragging) return;
			e.preventDefault();
			const x = e.pageX - ulWrap.offsetLeft;
			const walk = (x - startX) * 2;
			ulWrap.scrollLeft = scrollLeft - walk;
		});

		// Touch Events
		ulWrap.addEventListener('touchstart', (e) => {
			isDragging = true;
			startX = e.touches[0].clientX - ulWrap.offsetLeft;
			scrollLeft = ulWrap.scrollLeft;
		});

		ulWrap.addEventListener('touchend', () => {
			isDragging = false;
		});

		ulWrap.addEventListener('touchmove', (e) => {
			if (!isDragging) return;
			e.preventDefault();
			const x = e.touches[0].clientX - ulWrap.offsetLeft;
